...
- `he/index.html` — the Hebrew (RTL) version of the landing page, served at `/he/`. Marketing copy is translated; product-UI mockups stay in English (matching the app) inside `dir="ltr"` islands. Hebrew glyphs render in IBM Plex Sans Hebrew (`assets/fonts/plex-sans-hebrew-*.woff2`, declared with a Hebrew-only `unicode-range` so Latin text keeps using Archivo/Plex Mono). Both language versions cross-link via `hreflang` and nav/footer switchers.

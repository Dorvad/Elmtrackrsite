...
> **Branch consolidation (2026-07-16):** all branches were consolidated into a single `main` branch. `main` matches the live site exactly — none of this SEO/Hebrew work was published. The undeployed work is preserved in `main`'s history (folded in via `-s ours` merges), so the commits remain permanently reachable from `origin/main` even after the source branches are deleted. Preserved tips: `6263daa` (legal-pages: Hebrew `/he/`, canonical/hreflang/OG, expanded sitemap) and `43abb03` (cursor dev-env). To retrieve the Hebrew/SEO source for a later phase: `git show 6263daa:he/index.html`, `git show 6263daa:index.html`, `git show 6263daa:sitemap.xml`.
